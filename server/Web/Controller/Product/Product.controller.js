...
exports.Product_Create = function(req, res) {
   ...
   if(!ReceivingData.User_Id || ReceivingData.User_Id === '' ) {
      ...
      var ProductGroup = new ProductModel.ProductGroupsSchema({
         ...
         Creator_Type: 'Franchisee',
         User_Id : null,
         Variants: ReceivingData.Variants_List,
         HubUser_Id: null,
         Franchisee_Id: mongoose.Types.ObjectId(ReceivingData.User_Id),
         If_Deleted: false,
         Active_Status : true,
      });
      ...
      res.status(400).send({Status: false, Message: "User Details can not be empty" });
         ...
         ProductModel.ProductsSchema
            .find({ If_Deleted: false, $or: [ { Creator_Type: 'Admin' }, 
                                                { HubUser_Id: mongoose.Types.ObjectId(ReceivingData.HubUser_Id) },
                                                {Franchisee_Id: mongoose.Types.ObjectId(ReceivingData.User_Id)}
            ...
            if(err) {
               ...
               const CurrentStock = (info) => Promise.all([
                  FranchiseeStockModel.FranchiseeStockSchema.findOne({ Product_Id: info._id})
               ]).then(response => {
