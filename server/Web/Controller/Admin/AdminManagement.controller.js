      ...
      var ReceivingData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
       ...
       if (!ReceivingData.User_Id || ReceivingData.User_Id === ''  ) {
           res.status(400).send({Status: false, Message: "User Details can not be empty" });
              ...
              AdminModel.FranchiseeSchema
              .findOne({_id: ReceivingData.User_Id }, {Created_By: 1})
...
// -------------------------------------------------- Franchisee User Validate -----------------------------------------------
   exports.FranchiseeUser_Validate = function(req, res) {
      ...
      if(!ReceivingData.User_Name || ReceivingData.User_Name === '' ) {
         ...
         AdminModel.FranchiseeSchema.findOne( {  User_Name: { $regex : new RegExp("^" + ReceivingData.User_Name + "$", "i") },
                                                User_Password: ReceivingData.User_Password, 
                                                Active_Status: true 
                                             }, {Name: 1, User_Name: 1}, function(err, result) {
            if(err) {
               ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'Franchise User Find Query Error', 'AdminManagement.controller.js', err);
               res.status(417).send({status: false, Message: "Some error occurred while Find Franchisee User!."});
            } else {
               if ( result !== null) {
                  var ReturnData  = CryptoJS.AES.encrypt(JSON.stringify(result), 'SecretKeyOut@123');
                  ReturnData = ReturnData.toString();
